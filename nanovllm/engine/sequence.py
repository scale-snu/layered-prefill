...
class Sequence:
    ...
    def __getstate__(self):
        return (self.status, self.stage, self.num_stages, self.seq_id, self.num_tokens, self.num_prompt_tokens, self.num_processed_tokens, self.num_tokens_to_process, self.temperature, self.ignore_eos, self.max_tokens, self.block_table, self.error_message,
                self.token_ids if self.status in [SequenceStatus.WAITING, SequenceStatus.PREFILLING] else self.last_token)
