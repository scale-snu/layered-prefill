...
async def generate(request: Request) -> Response:
    ...
    text_outputs = prompt + final_output[1]
...
if __name__ == "__main__":
    ...
    for i in range(args.num_requests):
        prompt = "A long sequence of random tokens: " + " ".join(
            [str(np.random.randint(0, 10000)) for _ in range(args.random_input_len)]
        )
    ...
    async def run_all_requests():
        for i, results_generator in enumerate(results_generators):
            async for request_output in results_generator:
                text_outputs = request_output[1]
                print(f"Request {i} output: {text_outputs}")
